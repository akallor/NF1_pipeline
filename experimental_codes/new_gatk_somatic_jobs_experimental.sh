...
for sample_dir in JH-2-001; do
    if [ -d "$sample_dir" ]; then
        ...
        for bam_file in "$sample_dir"/CCDYYANXX_7_ATTACTCG-TATAGCCT_Aligned.out.sorted.dedup.bam; do
            if [ -f "$bam_file" ]; then
...
# 5. Filter Mutect2 Calls (Quality-based filters)
echo "Filtering Mutect2 calls with quality thresholds..."
...
# 6. SITE-LEVEL FILTERING: Apply site-specific filters for basic quality control
echo "Applying site-level filters for initial quality control..."
$gatk VariantFiltration \
    -R ${reference} \
    -V ${sample_dir}/${bam_base}.filtered.vcf \
    --filter-expression "DP < 10" --filter-name "SiteLowDepth" \
    --filter-expression "TLOD < 10.0" --filter-name "LowTLOD" \
    -O ${sample_dir}/${bam_base}.site_filtered.vcf

# 7. SAMPLE-LEVEL FILTERING: Apply sample-specific filters for depth and quality
echo "Applying sample-specific genotype filters..."
$gatk VariantFiltration \
    -R ${reference} \
    -V ${sample_dir}/${bam_base}.site_filtered.vcf \
    --genotype-filter-expression "DP < 10" --genotype-filter-name "SampleLowDepth" \
    --genotype-filter-expression "AF < 0.10" --genotype-filter-name "LowAlleleFreq" \
    -O ${sample_dir}/${bam_base}.fully_filtered.vcf

# 8. Extract only variants passing all site-level filters
echo "Extracting only PASS variants (site-level)..."
$bcftools view -f "PASS" ${sample_dir}/${bam_base}.fully_filtered.vcf > ${sample_dir}/${bam_base}.site_pass.vcf

# 9. Further filter to keep only variants passing both site and sample filters
echo "Filtering out variants with failed sample-level filters..."
$bcftools filter \
    -e 'FT ~ "SampleLowDepth" || FT ~ "LowAlleleFreq"' \
    -m x \
    ${sample_dir}/${bam_base}.site_pass.vcf > ${sample_dir}/${bam_base}.final.hc.vcf

# 10. Create a summary report for filtering
echo "Creating filtering summary report..."
echo "Total variants before filtering: \$(bcftools view -H ${sample_dir}/${bam_base}.filtered.vcf | wc -l)" > ${sample_dir}/${bam_base}.filter_summary.txt
echo "Variants failing site filters: \$(bcftools view -H ${sample_dir}/${bam_base}.site_filtered.vcf | grep -v "PASS" | wc -l)" >> ${sample_dir}/${bam_base}.filter_summary.txt
echo "Variants passing site filters: \$(bcftools view -H ${sample_dir}/${bam_base}.site_pass.vcf | wc -l)" >> ${sample_dir}/${bam_base}.filter_summary.txt
echo "Final high-confidence variants: \$(bcftools view -H ${sample_dir}/${bam_base}.final.hc.vcf | wc -l)" >> ${sample_dir}/${bam_base}.filter_summary.txt
