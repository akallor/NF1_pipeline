...
# Reference sample to filter
REFERENCE_SAMPLE="JH-2-014"

# Loop through directories greater than JH-2-014
for sample_dir in ../JH-2-*; do
    sample_name=$(basename "$sample_dir")
    
    # Check if the directory name is lexicographically greater than JH-2-014
    if [[ "$sample_name" > "$REFERENCE_SAMPLE" && -d "$sample_dir" ]]; then
